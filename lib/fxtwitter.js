...
const TAG = "[XEmissary:fxtwitter]";

export async function getTweet(id, debug = false) {
  ...
  for (const build of ENDPOINTS) {
    ...
    try {
      ...
      dlog("tried", url, "status", res.status, "bytes", text.length);
      if (!res.ok) {
      ...
      if (!tweet) {
        ...
        dlog("no tweet in response from", url, "top-level keys:", keys, "preview:", text.slice(0, 400));
...
function mapTweet(t, id) {
  ...
  const media = t.media || {};
  // GIFs are videos as far as FxTwitter is concerned (type "gif", with mp4 formats),
  // so they must never be picked up here too — otherwise they get attached twice.
  const isPhoto = (m) => m && (m.type ? /^(photo|image)$/i.test(m.type) : true);
  const photoSrc = media.photos || media.images || (media.all || []).filter(isPhoto);
  ...
  // Used as the Discord webhook avatar so the message shows the post's author.
  // Discord fetches this URL itself, so no host permission is needed for it.
  const avatarUrl =
    author.avatar_url || author.profile_image_url_https || author.profile_image_url || null;

  return { displayName, screenName, avatarUrl, text, postedIso, photos, videos, url, lang };
}
